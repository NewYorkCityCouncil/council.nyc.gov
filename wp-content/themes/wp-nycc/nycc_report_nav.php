...
?><div class="toc-nav row medium-collapse"><?php
    ...
    echo '<div class="columns medium-8 medium-push-2"><a class="button secondary small expanded" href="' . get_permalink($post->post_parent) . '" title="' . get_the_title($post->post_parent) . '">' . get_the_title($post->post_parent) . '</a></div>';

    ?><div class="columns small-6 medium-2 medium-pull-8"><?php
    if (!empty($prevID)) {
        echo '<a class="button secondary small expanded" href="' . get_permalink($prevID) . '" title="' . get_the_title($prevID) . '">Previous</a>';
    }
    ?></div><?php

    ?><div class="columns small-6 medium-2"><?php
    if (!empty($nextID)) {
